...
def hash(message, key):
    hashing = hmac.new(key, message, hashlib.sha256).hexdigest()
    return hashing

if __name__ == "__main__":
    modo = raw_input("Introduce el modo de confidencialidad/integridad (AutenticaCifra, AutenticaLuegoCifra, CifraAutentica): ")
    my_key = key_gen()
    IV = numberify(INIT_VEC)
    hashText = hash(EXAMPLE_TEXT, my_key)
    hashCiphered = encrypt(hashText, my_key, INIT_VEC)
    ...
    for block_num, (c1, c2) in enumerate(zip([IV]+blocks, blocks)):
        ...
        for i in xrange(15,-1,-1):
            for b in xrange(0,256):
                ...
                ##Aquí se simula que el texto cifrado llega al servidor para que este intente descifrarlo
                try:
                    if modo == "AutenticaCifra":
                        decrypt(stringify(c2), my_key, stringify(evil_c1))

                        if (hashText != hash(stringify(c2), my_key)):
                            print ("Mensaje del servidor: Error en el hash!")
                        
                    elif modo == "AutenticaLuegoCifra":
                        decrypt(stringify(c2), my_key, stringify(evil_c1))

                        decryptHash = decrypt(hashCiphered, my_key, INIT_VEC)
                        
                        if (decryptHash != hash(stringify(c2), my_key)):
                            print ("Mensaje del servidor: Error en el hash!")
                        
                    elif modo == "CifraAutentica":
                        if (hashText != hash(stringify(c2), my_key)):
                            print ("Mensaje del servidor: Error en el hash, abortando descifrado.")
                        
                        else:
                            decrypt(stringify(c2), my_key, stringify(evil_c1))
                        
                except InvalidPadding:
                    print "Mensaje del servidor: El padding no es correcto!"
                    pass
